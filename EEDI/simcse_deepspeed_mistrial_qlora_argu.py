...
class TrainDatasetForEmbedding(Dataset):
    def __init__(
        ...
        # misconception_mapping = pd.read_csv('../input/eedi-mining-misconceptions-in-mathematics/misconception_mapping.csv')
        # misconception_mapping['MisconceptionId'] = misconception_mapping['MisconceptionId'].map(lambda x: str(int(x)))
        # misconception_mapping = misconception_mapping.set_index('MisconceptionId')['MisconceptionName'].to_dict()
        # self.doc_dict = misconception_mapping
    ...
    def __getitem__(self, item) -> Tuple[str, List[str]]:
        ...
        # 不够 self.args.train_group_size - 1 ，重复取
        if len(self.dataset[item]['neg']) < self.args.train_group_size - 1:
            num = math.ceil((self.args.train_group_size - 1) / len(self.dataset[item]['neg']))
            negs = random.sample(self.dataset[item]['neg'] * num, self.args.train_group_size - 1)
        else:
            negs = random.sample(self.dataset[item]['neg'], self.args.train_group_size - 1)
        passages.extend([neg for neg in negs])
...
# from transformers import MistralModel, Qwen2Model
#
#
# class IgnoreLabelsWrapper(Qwen2Model):
...
def main():
    ...
    train_dataloader = DataLoader(train_dataset,
                                  ...
                                  collate_fn=EmbedCollator(tokenizer, query_max_len=args.query_max_len,
...
def parse_args():
    ...
    parser.add_argument('--lora_config', type=str, default='./configs/lora_config_llama.json',
    ...
    if args.gradient_checkpointing and args.lora_dim > 0:
        assert (
            not args.only_optimize_lora), '--gradient_checkpointing and --only_optimize_lora cannot be enabled at the same time.'
